...
contract('BAllowance', function (accounts) {
	...
	beforeEach(async () => {
		token = await BAllowance.new("Amplify", "ELEOS");
	});
	...
	it('checkBorrowAllowance fail', async () => {
		...
		await expectRevert(token.checkBorrowAllowance(user, other, TEST_AMOUNT), "Amplify: BORROW_NOT_ALLOWED");
		await token.borrowApprove(other, TEST_AMOUNT, {from: user});
		await expectRevert(token.checkBorrowAllowance(user, other, TEST_AMOUNT.add(new BN(1))), "Amplify: BORROW_NOT_ALLOWED");
