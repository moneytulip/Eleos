...
contract VaultToken is PoolToken {
    ...
    function _initialize(
        ...
        factory = msg.sender;
        _setName("Amplify Vault Token", "vELEOS");
        WETH = _router.WETH();
        router = _router;
        ...
        rewardsToken.safeApprove(address(router), type(uint256).max);
        WETH.safeApprove(address(router), type(uint256).max);
        underlying.safeApprove(address(masterChef), type(uint256).max);
    }

    /*** PoolToken Overrides ***/

    function _update() override internal {
